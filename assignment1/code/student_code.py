...
def hough_lines_vote_acc(edge_img, rho_res=1, thetas= np.arange(0,180)):
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    diagonal=int(np.sqrt(edge_img.shape[0]**2+edge_img.shape[1]**2))
    accumulator=np.zeros((diagonal,len(thetas)))
    height_y,width_x=np.where(edge_img==255)
    thetas_rad=np.deg2rad(thetas)
    rhos=np.arange(0,diagonal)
    for i in range(len(width_x)):
        for theta in thetas_rad:
            rho=np.int(np.ceil(width_x[i]*np.cos(theta)+height_y[i]*np.sin(theta)))
            accumulator[rho,np.int(np.rad2deg(theta))]+=1
    ### END OF STUDENT CODE ####
    ############################
...
def hough_circles_vote_acc(edge_img, radius):
    ...
    height_y,width_x=np.where(edge_img==255)
    accumulator=np.zeros(edge_img.shape)
    for i in range(len(width_x)):
        for t in theta:
            a=width_x[i]+r*np.cos(t)
            b=height_y[i]-r*np.sin(t)
            if a<edge_img.shape[1] and b<edge_img.shape[0]:
